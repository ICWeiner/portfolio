...
.github/workflows/ GitHub Actions (ci.yml PR checks, deploy.yml release)
```
...
- **Versioning:** on push to `main`, CI strips `-SNAPSHOT` to get the release
  ...
  `<revision>`/`version` fields when cutting a release (ADR-009).
- **Branches:** `main` is protected (PR + passing checks only); `develop` is
  the integration branch with direct pushes allowed. Feature branches merge
  into `develop`; releases ship via PR `develop` → `main` (ADR-008).
  `ci.yml` runs on PRs/pushes to `develop`, `deploy.yml` on push to `main`.
